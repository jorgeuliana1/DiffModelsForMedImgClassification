...
from aug_ndb import ImgTrainTransform, ImgEvalTransform

# Including the path to the models folder
sys.path.insert(0, os.environ["MY_MODELS_PATH"])
from my_model import set_model, get_norm_and_size  # noqa
...
def cnfg():
    ...
    save_dir = f"{comb_method}_{model_name}_fold_{folder}_{str(time.time()).replace('.', '')}"
...
def main(
    ...
    val_data_loader = get_data_loader(
        val_imgs_path, val_labels, val_meta_data,
        transform= ImgEvalTransform(*transform_param),
        batch_size= batch_size, shuf=True, num_workers=2, pin_memory=True
    )
    ...
    train_imgs_path = ["{}/{}".format(imgs_folder_train, img_id) for img_id in train_imgs_id]
    ...
    train_data_loader = get_data_loader(
        train_imgs_path, train_labels, train_meta_data,
    ...
    model = set_model(
        model_name, len(labels_name), neurons_reducer_block=neurons_reducer_block,
        comb_method=comb_method, comb_config=comb_config, pretrained=pretrained
    )
    ####################################################################################################################
    ...
    optimizer = optim.SGD(model.parameters(), lr=lr_init, momentum=0.9, weight_decay=0.001)
    scheduler_lr = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, factor=sched_factor, min_lr=sched_min_lr,
    ...
    print("-" * 50)
    fit_model(
        model, train_data_loader, val_data_loader, optimizer=optimizer, loss_fn=loss_fn, epochs=epochs,
        ...
        history_plot=True, val_metrics=["balanced_accuracy"], best_metric=best_metric
    )
    ####################################################################################################################
    ...
    test_model(
        model, val_data_loader, checkpoint_path=checkpoint_best, loss_fn=loss_fn, save_pred=True,
        partition_name='eval', metrics_to_comp='all', class_names=labels_name, metrics_options=metric_options,
    ...
    test_imgs_path = ["{}/{}".format(imgs_folder_train, img_id) for img_id in test_imgs_id]
    ...
    test_model(model, test_data_loader, checkpoint_path=checkpoint_best, metrics_to_comp="all",
    class_names=labels_name, metrics_options=metric_options, save_pred=True, verbose=False)
    ####################################################################################################################