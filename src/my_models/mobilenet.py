...
class MyMobilenet(nn.Module):
    ...
    def __init__(
        ...
        if comb_method is not None:
            if comb_config is None:
                raise Exception(
                    "You must define the comb_config since you have comb_method not None"
                )
            ...
            if comb_method == "metablock":
                ...
                raise Exception(
                    "There is no comb_method called "
                    + comb_method
        ...
        if neurons_reducer_block > 0:
            ...
            if comb_method == "concat":
                warnings.warn(
                    "You're using concat with neurons_reducer_block=0. Make sure you're doing it right!"
                )
    ...
    def forward(self, img, meta_data=None):
        ...
        if meta_data is not None and self.comb is None:
            raise Exception(
                "There is no combination method defined but you passed the metadata to the model!"
            )
        if meta_data is None and self.comb is not None:
            raise Exception(
                "You must pass meta_data since you're using a combination method!"
            )
        ...
        if self.comb == None:
            ...
            x = x.view(x.size(0), -1)  # flatting
            if self.reducer_block is not None:
                x = self.reducer_block(
                    x
                )  # feat reducer block. In this case, it must be defined
            ...
            x = x.view(x.size(0), self.comb_feat_maps, 8, 8).squeeze(
                -1
            )  # getting the feature maps
